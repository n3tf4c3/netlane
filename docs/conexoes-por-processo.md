...
### Validação complementar iniciada — 2026-09-10

O [novo ciclo de validação da interface](validacao-interface.md) acrescentou prova automatizada do `DispatcherTimer` real em uma janela WPF em memória, com fontes simuladas: atualização periódica, preservação da pesquisa/filtro/seleção/edições, retirada de dados após erro, recuperação automática e fim da coleta ao fechar. A suíte passou com **199 testes**, sem falhas ou ignorados. Na conferência do desktop, o usuário enviou uma captura da tela e confirmou que o horário avança sem clicar em **Atualizar**, encerrando a verificação de atualização automática por observação manual. Os demais itens interativos permanecem em andamento; o controle do Windows retornou `Computer Use native pipe is unavailable`.

As capturas seguintes confirmaram pesquisa por `onedrive`, limpeza da pesquisa, filtro **Com regra direta**, retorno a **Todos os processos** e tooltip da saída com interface/IP/contagem TCP. O filtro mostrou corretamente a regra desativada do Steam, preservando o significado de associação direta. Mais duas imagens confirmaram o tooltip do caminho completo e o foco visível na tabela; o usuário avaliou a janela como aparentemente OK após o roteiro de teclado e redimensionamento. A revisão básica está registrada, sem declarar cobertura de todo o percurso pelo teclado, de todas as transições de tamanho ou de outras escalas do Windows. A escala atual não foi informada.
